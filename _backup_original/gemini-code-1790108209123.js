    ...
    <div class="max-w-5xl mx-auto space-y-6">
        ...
        <div class="bg-slate-800 p-5 rounded-lg border border-slate-700 space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
                <input id="recName" placeholder="Subdomain (misal: crm)" class="bg-slate-900 border border-slate-700 px-3 py-2 rounded focus:outline-none focus:border-sky-500">
...
const server = http.createServer((req, res) => {
    ...
    if (req.method === 'POST' && req.url === '/api/records') {
        ...
        req.on('end', () => {
            try {
                ...
                res.end(JSON.stringify({ message: 'Record berhasil ditambahkan.' }));
            } catch (err) {
    ...
    if (req.method === 'POST' && req.url === '/api/rpz/sync') {
        try {
            ...
            res.writeHead(200, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify({ message: 'Proses sinkronisasi berjalan di background.' }));
        } catch (err) {
            res.writeHead(500, { 'Content-Type': 'application/json' });
            res.end(JSON.stringify({ error: err.message }));
        }
...
server.listen(PORT, '0.0.0.0', () => {
    console.log(`[+] Web Panel aktif di port ${PORT}`);
});